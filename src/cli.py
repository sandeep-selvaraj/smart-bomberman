...
def train_the_agent(existing_model_name: Optional[str] = None):
    ...
    if existing_model_name is not None:
        model = PPO.load(f"Training/{existing_model_name}", model='cuda',
                         env=env, tensorboard_log=log_path, gamma=0.99,
                         learning_rate=0.00001, ent_coef=0.01)
    # new_logger = configure(log_path, ["stdout", "csv", "tensorboard"])
    else:
        model = PPO("MlpPolicy", env, verbose=1, tensorboard_log=log_path, 
                    gamma=0.99, learning_rate=0.00001, ent_coef=0.01)
        # model = PPO("MultiInputPolicy", env, verbose=1, tensorboard_log=log_path, 
        #             gamma=0.99, learning_rate=0.0001, ent_coef=0.01)
    callback = CustomCallback(env=env)
    model.learn(total_timesteps=100000, callback=callback)
    now = datetime.datetime.now().strftime('%m%d_%H%M%S')
    model_path = os.path.join("Training", f"{now}.zip")
